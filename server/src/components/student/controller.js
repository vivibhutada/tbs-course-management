...
const getAll =
  ...
  async (req, res, next) => {
    try {
      ...
      if (error) {
        return commonHelper.sendErrorResponse(
          res,
          responseCode.validation.code,
          "fail",
          responseCode.validation.msg,
          error.details.map((err) => err.message)
        );
      }
      const result = await dao.getAll({ db, validation })(body);
      if (result.students.length > 0) {
        return commonHelper.sendSuccessResponse(
          res,
          200,
          "success",
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      if (req.files && req.files.length > 0) {
        req.body.files = req.files.map(
          (file) => req.params.folder_name + "/" + file.filename
        );
      ...
      if (req.files && req.files.length > 0) {
        ...
        for (const record of sheetData) {
          ...
          try {
            ...
            record.image = null;
            record.createdby = req.user.userid;
            let result = await dao.importFailRecordInsert({ db, validation })(
              record
            );
            failCount++;
            failedRecords.push({
        ...
        if (successCount > 0) {
          ...
          message +=
            successCount + " " + isAre + " " + "successfully inserted ";
        }
        if (failCount > 0) {
        ...
        return commonHelper.sendSuccessResponse(
          ...
          "success",
          message,
          { failCount: failCount, failedRecords: failedRecords }
        );
      } else {
        return commonHelper.sendErrorResponse(
          res,
          responseCode.success.code,
          "fail",
          validation.messages.no_file_uploaded
        );
      }
    } catch (error) {
      console.log("checklogin err==>>", error);
...
const getSampleFile =
  ...
  async (req, res, next) => {
    try {
      const filePath = commonHelper.generateSampleFilelink(
        "importSamplefiles/students-import-sample-file.xlsx"
      );
...
const exportFailedImportList =
  ({ dao, db, validation }) =>
  async (req, res, next) => {
    try {
      const result = await dao.getImportFailRecord({ db })(req.user.userid);
      if (result.failedRecords.length > 0) {
        const sanitizedRecords = result.failedRecords.map(
          ({ id, ...rest }) => rest
        );
        ...
        const filePath = path.join(
          process.env.STATIC_FILE_UPLOAD_PATH,
          "exports",
          fileName
        );
        console.log(filePath);
        ...
        const downloadlink = commonHelper.generateSampleFilelink(
          `exports/${fileName}`
        ); //
        return commonHelper.sendSuccessResponse(
          res,
          200,
          "success",
          validation.messages.export_fail_file,
...
const columnEncryptionDecryption =
  ({ dao, db, validation }) =>
  async (req, res, next) => {
    try {
      ...
      if (!table_name || !pk || !columns || !Array.isArray(columns) || !flag) {
        return commonHelper.sendSuccessResponse(
          ...
          200,
          "fail",
          "pk,table_name,flag and columns are required and can not be empty and columns must be in array ",
          []
        );
      }
      ...
      if (records.length > 0) {
        const updatedRecords = records.map((record) =>
          columns.reduce(
            (updatedRecords, column) => {
              if (column in record) {
                if (flag === "encrypt") {
                  updatedRecords[column] = commonHelper.valueEncryption(
                    record[column]
                  );

                  //,
                } else if (flag === "decrypt") {
                  updatedRecords[column] = commonHelper.valueDecryption(
                    record[column]
                  );
                }
              }
    ...
    try {
      ...
      if (result) {
        return commonHelper.sendSuccessResponse(
          ...
          200,
          "success",
          validation.messages.statusupdate
        );
      } else {
        return commonHelper.sendSuccessResponse(
          res,
